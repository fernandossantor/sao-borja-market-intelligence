# 00 — LEIA-ME — Pacote Editorial Corrente — SBMI — v003

**Data:** 25/09/2026

## Prioridade

A prioridade do pacote é a qualidade dos cinco cadernos em Google Docs: profundidade, consistência argumentativa, demonstração, rastreabilidade de fonte e raciocínio, especificidade e utilidade decisória. Gráficos e PDF não são requisitos de fechamento.

## Publicações correntes

- Caderno Geral v004 — `13yqwDHKJuEag4I0l7XaUZ_oDIg5lh0UvQsmJWc3H2Co`
- Bens Essenciais v003 — `190arYFyxKq_xynAbqU3fGThlpbJiqPXlmqMOrtRHss0`
- Saúde/Higiene v003 — `1d141fQT3aTMtHoj5el9FOlR0mXqTgcNp2E7QqV28ZjI`
- Bens Não Essenciais v003 — `1uBIiR9pH6wqzIH2T5nO_dRpDEu2Cu-qP3v4cCcs2DDE`
- Alimentação Fora do Lar e Serviços v003 — `1pyUYH5APuuRR72XZ9Pd1BjpQDcgDQ1yTNActUkium3Y`

## Organização no Drive

`cadernos_editoriais_em_producao/`

- `01_caderno_geral/`
- `02_bens_essenciais/`
- `03_saude_higiene/`
- `04_bens_nao_essenciais/`
- `05_alimentacao_servicos/`
- `06_qa_editorial/`

## QA corrente

Drive: `1wqU0Bhy8RpkXuYr3K7BoORqHo6sxtOddFtf-2ZkigZw`

GitHub: `docs/governance/qa_argumentativo_cadernos_20260925_v002.md`

Status: **APROVADO PARA REVISÃO HUMANA FINAL.**

## Padrão editorial

Drive: `1Lx4AOMN1hY8_s0qMaM_TUSLSHv1YrT3dlV-BW0lOEUk`

GitHub: `docs/governance/padrao_editorial_decisorio_cadernos_20260925_v001.md`

Regra: `evidência → contexto → relação → mecanismo → interpretação → implicação → decisão → indicador → limite`.

## Base técnica

- Caderno-Base v029 — `1Fj6MgIPDetaN8PPUeJ7k8zsH5L6jeHoClmb3ZST9oD4`
- Planilha técnica v029 — `1CHn1JZ-IcDxG3V9M5y0PKvN5lTkcvlVcov_vVw3je3c`
- Registro Metodológico v029 — `1aHHbEBLjJKTp61zkdH7ysSz3rrvkfRA8-Iv3KvYtC2w`
- quatro bases técnicas setoriais v002.

O Caderno-Base v028 permanece read-only.

## Próxima etapa

Executar leitura humana integrada dos cinco cadernos, removendo somente repetição sem ganho argumentativo, problemas de ritmo, divergências terminológicas, referências incompletas e falhas de sumário/estilo. Não reduzir demonstração ou profundidade para obter concisão artificial.

## Governança

PR #41 permanece aberto, draft e sem merge. Nenhum merge ou retarget sem autorização explícita.
