...
SUPPORTED_EXTENSIONS = SUPPORTED_EXCEL | SUPPORTED_DELIMITED
YEAR_PATTERN = re.compile(r"(?<!\d)(?:18|19|20|21)\d{2}(?!\d)")
INTEGER_PATTERN = re.compile(r"[-+]?\d+")
DECIMAL_PATTERN = re.compile(r"[-+]?(?:\d+[.,]\d+|\d{1,3}(?:\.\d{3})+,[0-9]+)")
DATE_TEXT_PATTERNS = (
...
def _years_from_value(value: object) -> list[int]:
    if isinstance(value, (datetime, date)):
        return [value.year]
    return [int(match.group()) for match in YEAR_PATTERN.finditer(str(value or ""))]
...
def _classify_text(value: str) -> str:
    ...
    if any(pattern.fullmatch(text) for pattern in DATE_TEXT_PATTERNS):
        return "date_text"
    if INTEGER_PATTERN.fullmatch(text) and not (len(text.lstrip("+-")) > 1 and text.lstrip("+-").startswith("0")):
        return "integer_text"
...
def classify_value(value: object, *, data_type: str | None = None, is_date: bool = False) -> str:
...
def _header_candidate(rows: list[list[CellObservation]], max_scan_rows: int = 25) -> tuple[int | None, str]:
    ...
    for row_index, row in enumerate(rows[:max_scan_rows], start=1):
        ...
        if not nonblank:
            continue
        text_like = sum(cell.kind in {"text", "date_text"} for cell in nonblank)
        ...
        score = (float(len(nonblank)), text_ratio, unique_ratio, float(-row_index))
...
def _header_values(
    ...
    if header_row is None:
        return [f"column_{index}" for index in range(1, observed_max_column + 1)]
...
def _schema_signature(headers: list[str]) -> str:
    normalized = [normalize_label(value) or f"__blank_{index}" for index, value in enumerate(headers, start=1)]
...
def _profile_table(
    *,
    ...
    for row_index, row in enumerate(rows, start=1):
        nonblank_positions = [index for index, cell in enumerate(row, start=1) if cell.kind != "blank"]
        if not nonblank_positions:
            continue
        observed_nonempty_rows += 1
        observed_nonempty_cells += len(nonblank_positions)
        observed_max_column = max(observed_max_column, max(nonblank_positions))
    ...
    duplicate_header_labels = sum(count - 1 for count in Counter(nonblank_headers).values() if count > 1)
    ...
    for column_index in range(1, observed_max_column + 1):
        ...
        for row in rows[data_start:]:
            cell = row[column_index - 1] if column_index <= len(row) else CellObservation(None, "blank")
            counts[cell.kind] += 1
            if cell.kind != "blank":
        ...
        column_records.append(
            {
                ...
                "header_observed": headers[column_index - 1] if column_index <= len(headers) else "",
                "header_normalized": normalized_headers[column_index - 1]
                if column_index <= len(normalized_headers)
                else "",
                "nonempty_count": int(sum(counts.values()) - counts.get("blank", 0)),
...
def _excel_tables(path: Path, relative_path: str) -> Iterable[tuple[dict[str, object], list[dict[str, object]]]]:
...
def _delimited_table(path: Path, relative_path: str) -> tuple[dict[str, object], list[dict[str, object]]]:
    ...
    rows = [
        _trim_row([CellObservation(value, classify_value(value)) for value in source_row])
...
def _schema_groups(sheets: pd.DataFrame) -> pd.DataFrame:
    ...
    repeated.insert(
        1,
        "group_size",
        repeated.groupby("schema_signature_sha256")["relative_path"].transform("size").astype(int),
    )
    return repeated.sort_values(
        ["group_size", "schema_signature_sha256", "relative_path", "sheet_name"],
...
def profile_snapshot(snapshot_path: Path) -> ProfileResult:
    ...
    if not inbox_root.is_dir():
        raise FileNotFoundError(f"Caixa capturada não encontrada: {inbox_root}")

    file_records: list[dict[str, object]] = []
    sheet_records: list[dict[str, object]] = []
    column_records: list[dict[str, object]] = []

    for path in sorted(candidate for candidate in inbox_root.rglob("*") if candidate.is_file()):
        relative_path = PurePosixPath(*path.relative_to(root).parts).as_posix()
        ...
        except Exception as exc:  # noqa: BLE001 - erro é registrado por arquivo e a auditoria continua
